...
import { moodHistoryHeaderData } from "../components/MoodTracker/MoodData";
import { Select, SelectOptions } from "../components/shared/Select";
import { moodEmojis } from "../components/shared/MockData";
...
export const MoodTrackerPage = () => {
  ...
  const MoodTrendFilter = () => {
    return (
      <select
        className="bg-white border p-2 rounded-lg font-medium text-sm"
        name="filter"
        id="filter"
      >
  ...
  return (
    <>
      ...
      <EmojiBanner
        updateHistory={() => setHistoryUpdated((prev) => prev + 1)}
        ref={logMoodBtn}
      />
      <Card cardTitle="Mood Trend" cardAction={<MoodTrendFilter />}>
        <BarChart
          emptyState={
            <EmptyState
              header={
                isFetching ? "Mood Trend" : "Your Mood Trend is Unavailable"
              }
              subheader={
                isFetching
                  ? `Mood trend is loading...`
                  : `Start logging your mood to get access to your mood trend`
              }
              loading={isFetching}
      ...
      <Card cardTitle="Mood History" cardAction={<MoodHistoryFilter />}>
        <Table
          ...
          emptyState={
            <EmptyState
              header={
                isFetching ? "Mood History" : "Your Mood History is Empty"
              }
              subheader={
                isFetching
                  ? `Mood history is loading...`
                  : `When you start logging your moods you’ll be able to view your mood history`
              }
              loading={isFetching}
              onClick={() => setLogMoodClicked((prev) => prev + 1)}
              btnText="Log Mood"
