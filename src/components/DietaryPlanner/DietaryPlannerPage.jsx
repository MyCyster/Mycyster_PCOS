...
export const DietaryPlannerPage = () => {
                    ...
                    <SelectOptions>
                        <form action={handleDPFilter} className='flex flex-col gap-4'>
                            ...
                            <Button className="flex items-center justify-center gap-2 bg-primary text-white border border-primary rounded-xl font-bold text-base w-full mt-4 px-0 hover:shadow-lg hover:bg-primary-300">
                                <ClipLoader color="#069494" size={16} loading={isDPFilterLoading}/>
                                Filter
                            </Button>
                        </form>
                    </SelectOptions>
    ...
    return (
        <>
            ...
            <CarouselCard actions={<DietaryMealActions/>} title={`Today's meal plan`}>
                {meals.length ? 
                    <>
                    ...
                    <EmptyState
                        header='Meal Plan'
                        subheader={isCalorieLoading ? `Meal plan is loading...` : `Fill the meal survey to help us suggest pre-made meal plans for you`}
