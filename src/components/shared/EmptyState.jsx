...
export const EmptyState = ({header, subheader, loading, onClick, btnText}) => {
    return (
        <div className='w-1/2 m-auto my-8 text-center'>
            ...
            <p className='font-medium mb-10'>{subheader}</p>
            <svg className='m-auto' width="201" height="128">
                <use xlinkHref="/icon-sprite.svg#emptyMoodState" />
            </svg>
            {!loading && <Button onClick={onClick} className="bg-primary text-white border border-primary rounded-xl font-medium text-base mt-6 hover:shadow-lg hover:bg-primary-300">
                {btnText}
            </Button>}
        </div>
    )
