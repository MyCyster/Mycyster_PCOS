  ...
  const submitMood = async (formData) => {
    ...
    try {
      const response = await fetch(
        "https://mycyster-backend.onrender.com/v1/mood-tracker",
        {
          method: "POST",
          headers: {
      ...
      <div className="grid grid-cols-5 justify-between w-4/5">
        {moodEmojis.map((moodEmoji, i) => (
          <div key={i}>
            ...
            >
              <svg size={100} width={moodEmoji.width} height={moodEmoji.height}>
                <use xlinkHref={`/icon-sprite.svg#${moodEmoji.emoji}`} />
              </svg>
              {/* I changed this text-xl to base */}
              <p className="text-base font-medium text-white mt-2 break-words w-11/12 text-center">
                {moodEmoji.name}
              </p>
            </label>
          </div>
